...
async def experience_alg(main_questions,secondary_questions,**kwargs):
    ...
    Types =[ {
      "key": "Experiente",
      **copy.deepcopy(CATEGORY_CONFIG["Experiente"])
    }]

    remove_types(Types,indoor,wheater,nearby_excluded_types)
    print("asteas types",Types)
...
def remove_types(Types,indoor,wheater,banned_types):
    ...
    types=Types[0]
    for type in types["nearby_search_non_prim_types"]+types["nearby_type"]+types["text_query"]:
    ...
    print("asta e banned types", to_remove)
    lst = types["nearby_search_non_prim_types"]
    print("asta e nearby search non prim:",lst)
