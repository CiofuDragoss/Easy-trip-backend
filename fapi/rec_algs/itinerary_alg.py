...
async def itinerary_alg(main_questions, sec_q, **kwargs):
    ...
    day = sec_q.day
    circle = Circle(
        center=Center(latitude=latitude, longitude=longitude), radius=distance
    )
    loc_restr = LocationRestriction(circle=circle)
    yield {"stage": "pas 1", "info": "itinerariul tau se genereaza..."}
    visited_instance, global_set = await get_visited_set(auth)
    shopping_first = (
        random.choice([True, False])
        if itinerary_type == "Dimineata" and shopping
        else False
    )
    try:
        ...
        visited_instance.place_ids.clear()
        visited_instance.update_index = 0
        await visited_instance.save()
        raise e
    yield {"stage": "pas 2", "info": "mai avem putin..."}
    ...
    yield {"stage": "final", "data": {**result_first_period, **result_second_period}}
